...
ws.column_dimensions['D'].width = 26   # 對客通知
ws.column_dimensions['E'].width = 28   # 批次對客通知
ws.column_dimensions['F'].width = 26   # 一般工單
ws.column_dimensions['G'].width = 20   # 差異說明

# ── 標題列 ──
ws.row_dimensions[1].height = 32
for col, h in enumerate(['步驟','階段','額滿挽單','對客通知（單筆）','批次對客通知','一般工單','差異說明'], 1):
...
# ── 流程資料 (步驟, 階段, 額滿挽單, 對客通知, 批次對客通知, 一般工單, 差異說明) ──
rows = [
    # 零、輸入方式
    ('—', '零、輸入方式',
     '手動輸入\n（訂單編號）',
    ...
    # 一、登入
    (1,  '一、登入',
     '登入 BE2', '登入 BE2', '登入 BE2（逐筆執行）', '登入 BE2', ''),
    ...
    # 三、建立工單
    (3,  '三、建立工單',
     '前往客服操作台', '前往客服操作台', '前往客服操作台', '前往客服操作台', ''),

    (4,  '',
     '點擊「新增工單」', '點擊「新增工單」', '點擊「新增工單」', '點擊「新增工單」', ''),
...
# ── 寫入 ──
for r_idx, (step, stage, w, n, b, g, diff) in enumerate(rows, 2):
    is_input_row = (step == '—')
    ws.row_dimensions[r_idx].height = 52 if is_input_row else 46
